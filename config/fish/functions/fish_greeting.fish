function fish_greeting
    ...
    set INFO_LAST_LOGIN $(echo 'Last login: '(set_color white)(last -F -n 1 -R $USER | head -n 1 | awk '{$1=$2=$3=$NF=""; print $0}' | xargs echo))

    set INFO_DATE $(echo 'Date:       '(set_color white)(date +"%A, %b %d, %Y"))
    set INFO_TIME $(echo 'Time:       '(set_color white)(date +"%R, %Z %:z"))
    set INFO_UPTIME $(echo 'Uptime:     '(set_color white)(uptime -p | sed 's/^[^up]*up //'))

    set INFO_HARDWARE $(echo 'Hardware:   '(set_color white)(hostnamectl | grep "Hardware Model: " | sed 's/[^:]*:[[:space:]]*//'))
    set INFO_OS $(echo 'OS:         '(set_color white)(hostnamectl | grep "Operating System: " | sed 's/[^:]*:[[:space:]]*//'))
    set INFO_VERSION $(echo 'Kernel:     '(set_color white)(hostnamectl | grep "Kernel: " | sed 's/[^:]*:[[:space:]]Linux *//'))

    set_color --dim --italics 808080
    echo -e $INFO_LAST_LOGIN '\n'
    set_color normal

    echo 'Welcome back, '(set_color --bold $MAIN_COLOUR)(whoami)'@'(hostname)(set_color normal)(set_color white)', ya silly 🪿💨💨💨'
